...
def ea(population, toolbox, cxpb, mutpb, elitism, ngen, fitness, stats=None,
    ...
    for gen in range(1, ngen + 1):
        population = eval_population(population, toolbox, fitness)
...
def eval_population(population, toolbox, fitness):
    inds = [ind for ind in population]
    strs = [str_ind(ind) for ind in inds]
    for i, ind in enumerate(inds):
        ind.str = strs[i]
    results = toolbox.map(partial(fitness.worker_func, tree_compiler=toolbox.compile), strs)
